...
var S = class {
  ...
  dive() {
    ...
    if (y(n) === F) return { done: false, value: this.result() };
    const o = e.get(y(n));
...
var gt = (t, e, n) => {
  ...
  for (let u = 1; u < i; ++u) r[u * s] = u;
  return G(t, e, n, o, r, 1, s, ""), o;
};
var G = (t, e, n, o, s, i, r, u) => {
  const c = i * r;
  t: for (const d of t.keys()) if (d === F) {
    const a = s[c - 1];
...
var A = class _A {
  ...
  atPrefix(e) {
    ...
    if (n === void 0) {
      const [s, i] = k(o);
      for (const r of s.keys()) if (r !== F && r.startsWith(i)) {
...
var E = (t, e, n = []) => {
  ...
  for (const o of t.keys()) if (o !== F && e.startsWith(o)) return n.push([t, o]), E(t.get(o), e.slice(o.length), n);
  return n.push([t, e]), E(void 0, "", n);
};
var v = (t, e) => {
  if (e.length === 0 || t == null) return t;
  for (const n of t.keys()) if (n !== F && e.startsWith(n)) return v(t.get(n), e.slice(n.length));
};
var b = (t, e) => {
  const n = e.length;
  t: for (let o = 0; t && o < n; ) {
    for (const i of t.keys()) if (i !== F && e[o] === i[0]) {
...
var nt = (t, e, n, o) => {
  ...
  let i = s.get(e);
  if (i == null) i = /* @__PURE__ */ new Map(), i.set(n, 1), s.set(e, i);
  else {
    const r = i.get(n);
    i.set(n, (r || 0) + 1);
  }
...
var T = (t, e) => {
  ...
  if (u == null) throw new Error(`SlimSearch: document does not have ID field "${r}"`);
  if (Y(t, u)) throw new Error(`SlimSearch: duplicate ID ${u}`);
  const c = It(t, u);
  Dt(t, c, e);
  for (const d of i) {
    const a = n(e, d);
    ...
    for (const g of h) {
      const l = s(g, d);
      if (Array.isArray(l)) for (const m of l) nt(t, f, c, m);
      else l && nt(t, f, c, l);
    }
  }
};
var L = (t, e) => {
  for (const n of e) T(t, n);
};
var St = (t, e, n = {}) => {
...
var $ = { ...j, ...q };
var kt = (t) => {
  if (B.hasOwnProperty(t)) return V(B, t);
...
var N = (t, e, n, o, s, i, r, u, c = /* @__PURE__ */ new Map()) => {
  ...
  for (const d of Object.keys(i)) {
    ...
    for (const l of f.keys()) {
      ...
      } else c.set(l, { score: z, terms: [e], match: { [n]: [d] } });
    }
  }
  return c;
};
...
var Mt = (t, e, n = {}) => {
  ...
  const s = [];
  for (const [i, { score: r, terms: u, count: c }] of o) s.push({ suggestion: i, terms: u, score: r / c });
  return s.sort(Z), s;
};
var rt = class {
  constructor(e) {
  ...
  toJSON() {
    ...
    for (const [n, o] of this._index) {
      const s = {};
      for (const [i, r] of o) s[i] = Object.fromEntries(r);
      e.push([n, s]);
    }
...
var ct = ({ index: t, documentCount: e, nextId: n, documentIds: o, fieldIds: s, fieldLength: i, averageFieldLength: r, storedFields: u, dirtCount: c, serializationVersion: d }, a) => {
  if (d !== 1 && d !== 2) throw new Error("SlimSearch: cannot deserialize an index created with an incompatible version");
  const h = new rt(a);
  ...
  for (const [f, _] of h._documentIds) h._idToShortId.set(_, f);
  for (const [f, _] of t) {
...
var Lt = (t, e) => {
  if (e == null) throw new Error("SlimSearch: loadJSON should be given the same options used when serializing the index");
...
var lt = async (t, e, n) => {
  ...
  if (dt(t, n)) {
    ...
    for (const [u, c] of t._index) {
      for (const [d, a] of c) for (const [h] of a) t._documentIds.has(h) || (a.size <= 1 ? c.delete(d) : a.delete(h));
...
var mt = (t, e) => {
  ...
  if (c == null) throw new Error(`SlimSearch: cannot remove document with ID ${u}: it is not in the index`);
  for (const d of i) {
    const a = s(e, d);
    if (a == null) continue;
    ...
    for (const g of h) {
      const l = o(g, d);
      if (Array.isArray(l)) for (const m of l) M(t, f, c, m);
      else l && M(t, f, c, l);
    }
  }
